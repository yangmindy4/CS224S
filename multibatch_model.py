...
def parse_data():
    ...
    for i in range(total_songs):
        mfcc_counter = 0
        while True:
            soundfile_name = "data/soundmfcc2/" + str(i).zfill(4) + "-" + str(mfcc_counter) + ".npy"
            lyricsfile_name = "data/lyricsParsed/" + str(i).zfill(4) + "-" + str(mfcc_counter) + ".txt"
            if not os.path.exists(soundfile_name):
                break
            mfcc_features = np.load(soundfile_name).T

            lyrics = open(lyricsfile_name, 'r').read().lower().replace('\n', ' ').replace('&#39;', '').replace('-', ' ').replace(',', '').replace('&QUOT;', '').replace('?', '')
            ...
            lyrics = np.hstack([SPACE_TOKEN if x == ' ' else list(x) for x in lyrics])
            # Transform char into index
            lyrics = np.asarray([SPACE_INDEX if x == SPACE_TOKEN else ord(x) - FIRST_INDEX for x in lyrics])
            neg = False
            for x in lyrics:
                if x < 0:
                    neg = True
    ...
    return np.asarray(inputs), np.asarray(labels), num_examples
...
inputs, labels, num_examples = parse_data()
